export default function Header({ meta }) {
  const total    = meta?.total_skills ?? '—'
  const domains  = meta ? Object.values(meta.by_domain).filter(n => n > 0).length : '—'
  const stars    = meta?.stars ?? 0
  const repo     = meta?.repo ?? 'le-diegz/claude-skills-registry'
      ...
      <div className="header-inner">
        <div className="header-brand">
          <span className="header-logo">
            claude<span>-</span>skills<span>-</span>registry
          </span>
        </div>
        ...
        <a href={ghUrl} target="_blank" rel="noopener noreferrer" className="header-github">
